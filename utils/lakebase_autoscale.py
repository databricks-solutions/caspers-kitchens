...
def delete_postgres_database(
    ...
    *,
    ...
    """Best-effort delete of a Postgres database in a project branch.

    Returns True if a delete call was issued, False if the database was
    already absent.  Re-raises any non-404 error.
    """
    name = _database_resource_name(project_id, database_id, branch_id=branch_id)
    try:
        w.postgres.delete_database(name=name)
        return True
    except Exception as e:
        if _is_not_found_v2(e):
            return False
...
def recreate_postgres_database(
    ...
    *,
    ...
    """Drop the database if present, wait for the delete to finalise, then
    create it fresh.

    Returns ``(database_dict, existed_before)`` where ``existed_before`` is
    True when an old database was dropped.  Deletion is asynchronous, so we
    poll ``get_postgres_database`` until it reports the DB is gone before
    recreating (recreating too early races with the in-flight delete).
    """
    import time as _time

    existed = delete_postgres_database(
        w, project_id=project_id, database_id=database_id, branch_id=branch_id,
    )
    if existed:
        deadline = _time.time() + wait_s
        while get_postgres_database(
            w, project_id=project_id, database_id=database_id, branch_id=branch_id,
        ) is not None:
            if _time.time() > deadline:
                raise TimeoutError(
                    f"Database {database_id!r} still present {wait_s}s after "
                    f"delete in projects/{project_id}/branches/{branch_id}"
                )
            _time.sleep(5)
