...
# MAGIC %md
# MAGIC # Casper's Kitchens - Streaming Data Replay
# MAGIC
# MAGIC This notebook replays pre-generated ghost kitchen events using a custom PySpark streaming data source.
# MAGIC
# MAGIC **Features:**
# MAGIC - Reads from single self-contained events.parquet file (34 MB)
# MAGIC - Uses checkpoint to track progress
# MAGIC - Supports variable speed multiplier (1x, 60x, 3600x, etc.)
# MAGIC - Works with `availableNow` trigger for scheduled runs
# MAGIC
# MAGIC **Dataset:**
# MAGIC - 75,296 orders across 4 cities (SF, SV, Bellevue, Chicago)
# MAGIC - 1,008,422 events (order lifecycle + driver tracking)
# MAGIC - Real OpenStreetMap routes
...
# MAGIC %md
# MAGIC ## 2. Custom Data Source Definition
# MAGIC
# MAGIC This is a self-contained streaming data source - all logic inline.
...
class CaspersStreamReader(DataSourceStreamReader):
    ...
    Each run processes: (current_real_time - checkpoint_time) × speed_multiplier
    """
...
# Configuration
DATASET_PATH = "/dbfs/caspers/canonical_dataset"  # Update this path where you uploaded the dataset
START_DAY = 70  # Day 0-89 available (day 70 = early March in the simulation)
SPEED_MULTIPLIER = 60.0  # 60x realtime (1 real minute = 1 sim hour)
...
# MAGIC %md
# MAGIC ## 5. Write Stream (availableNow)

# COMMAND ----------

# Output configuration
OUTPUT_PATH = "/dbfs/caspers/streaming_output"
CHECKPOINT_PATH = "/dbfs/caspers/checkpoint"
...
print("🚀 Streaming query started with availableNow trigger")
...
# MAGIC %md
# MAGIC ## 6. Query Results (Optional)

# COMMAND ----------

# Read the output JSON files
events_df = spark.read.format("json").load(OUTPUT_PATH)

print(f"📊 Total events processed: {events_df.count():,}")

# Show summary by location
print("\n🌍 Events by location:")
events_df.groupBy("location_id").count().orderBy("location_id").show()

# Show summary by event type
print("\n📈 Events by type:")
events_df.groupBy("event_type").count().orderBy("event_type").show()

# Show sample events
print("\n📄 Sample events:")
events_df.limit(10).show(truncate=False)
...
# MAGIC
# MAGIC **How It Works:**
# MAGIC - Each scheduled run processes events based on real time elapsed since last checkpoint
# MAGIC - Checkpoint stores: `{simulation_seconds: Unix timestamp, offset_timestamp: ISO string}`
# MAGIC - Formula: `new_events = (current_time - checkpoint_time) × speed_multiplier`
# MAGIC
# MAGIC **Speed Multiplier Examples:**
# MAGIC - `1.0` = realtime (1 real second = 1 sim second)
# MAGIC - `60.0` = 60x speed (1 real minute = 1 sim hour) ← Good default for scheduled runs
# MAGIC - `3600.0` = 3600x speed (1 real hour = 1 sim 150 days - processes entire dataset quickly)
# MAGIC
# MAGIC **Scheduling:**
# MAGIC - Schedule this notebook to run at any interval (5 min, 10 min, irregular)
# MAGIC - Each run automatically processes the right amount of data
# MAGIC - Works with random/chaotic intervals - checkpoint tracks everything
# MAGIC
# MAGIC **Dataset Files Needed:**
# MAGIC - `events.parquet` (34 MB) - Self-contained with all event data
# MAGIC - Total: ~34 MB
# MAGIC
# MAGIC **Schema Changes:**
# MAGIC - `gk_id` removed (was fake data)
# MAGIC - `location_code` replaced with `location_id` (resolve in downstream demos)
# MAGIC - `order_id` is now random 6-char alphanumeric (e.g., "A7K2M9")
